...
static const char*help_titles[]={"実行・ステップ","Source / Assembly","閲覧履歴","停止条件","Variables","Registers / Stack","VIM操作","画面の記号","終了"};
static const char*help_execution[]={"r   プログラムを実行","c   停止位置から継続","n   次のソース行まで進む","s   関数内へ入る。行情報なしならAssemblyへ自動切替","f   現在の関数がreturnするまで実行","    呼び出し元に行情報あり: Sourceへ戻る","    呼び出し元に行情報なし: Assemblyのまま","    最外フレーム: プログラム終了まで継続","i/I Assemblyで1命令進む / callを飛び越す","R   現在の関数を強制return（確認あり）",NULL};
static const char*help_source[]={"d   Source / Disassemblyを手動切替","e   現在の実行位置へ戻る（実行状態は変更しない）","o   Function / Fileからソースを開く","=>  現在の実行位置",">   閲覧カーソル","停止時のframeにfullname + lineがあればSource Mode","行情報なし、またはソースを読めなければAssembly Mode","別ファイルへstepした場合も自動的に表示を切り替える",NULL};
static const char*help_history[]={"[ または Ctrl-o   閲覧履歴を戻る","]                  閲覧履歴を進む","履歴移動は表示だけを変え、プログラムを実行しない","f は履歴移動ではなく、関数のreturnまで実行する","Ctrl-iは端末上でTabと同じため、進む操作には]を使用",NULL};
static const char*help_stops[]={"b   ソース行 / 命令アドレスのBreakpoint","B   条件付きBreakpoint","F   関数名からFunction Breakpoint","C   Catchpoint（syscall/signal/fork/vfork/exec/load）","w   Watchpoint","L   停止条件一覧","一覧: j/k 選択  d 削除  e 有効/無効  Enter 移動","ID: B# Breakpoint  C# Catchpoint  W# Watchpoint",NULL};
static const char*help_variables[]={"Tab / Shift-Tab   ペインを前後移動","j/k     変数を選択","Enter   構造体・ポインタを1階層展開 / 折りたたみ","p       値を詳細表示","a       Address / Pointer Details","E       値を変更（確認あり）","w       選択中の式にWatchpoint","B       選択中の式で条件付きBreakpoint","ArgsとLocalsは現在選択中のStack frame固有",NULL};
static const char*help_registers[]={"Registers: j/k 選択  p 詳細  E 値変更","Stack:     j/k 選択  Enterでframe切り替え","[P] プロジェクト内ソース","[X] 外部ライブラリのソース","[A] ソース情報なし（address only）","syscall Entry: orig_raxとarg1〜arg6を表示","syscall Return: raxの戻り値と引数を表示",NULL};
static const char*help_vim[]={"F2   GDB操作モード / VIM操作モード切り替え","h/j/k/l  w/b/e  0/^/$  gg/G  H/M/L","PageUp/PageDown  Ctrl-u/Ctrl-d  Ctrl-f/Ctrl-b","/ 前方検索  ? 後方検索  n/N 次・前候補","* / #   カーソル上の単語を前方 / 後方検索","gd / gD ローカル / ファイル先頭から宣言検索","% 対応括弧  {/} 段落移動  zz/zt/zb 画面位置","VIM操作モードは読み取り専用",NULL};
static const char*help_symbols[]={"[SRC] Source Mode    [ASM] Assembly Mode","[GDB] GDB操作        [VIM] VIM操作","[EXEC] 実行位置表示  [VIEW] 閲覧位置表示","=> 実行位置          > 選択カーソル","シアン: Breakpoint   黄色: 条件付き/Catchpoint","緑: Watchpoint・実行位置  赤: エラー  暗色: 無効","STOPS行にB/C/Wの件数と最後に停止した条件を表示",NULL};
static const char*help_quit[]={"O   プログラム出力を表示","Ctrl-r ソースファイルを再読み込み","q   gdを終了","実行中または停止中は終了確認を表示","Esc または ? でHelpを閉じる","hもGDB操作モードではHelpの別名として使用可能",NULL};
static const char*const*help_pages[]={help_execution,help_source,help_history,help_stops,help_variables,help_registers,help_vim,help_symbols,help_quit};
#define HELP_TOPIC_COUNT ((int)(sizeof(help_titles)/sizeof(help_titles[0])))
static void draw_help(int selected){int rows,cols;getmaxyx(stdscr,rows,cols);erase();attron(A_BOLD|COLOR_PAIR(COLOR_INFO));clipped(0,2,cols-4,"gd Help");attroff(A_BOLD|COLOR_PAIR(COLOR_INFO));int cell=cols/3;for(int i=0;i<HELP_TOPIC_COUNT;i++){int y=2+i/3,x=1+(i%3)*cell;if(i==selected)attron(A_REVERSE|A_BOLD);clipped(y,x,cell-2,"%c %s",i==selected?'>':' ',help_titles[i]);if(i==selected)attroff(A_REVERSE|A_BOLD);}line_rule(6,help_titles[selected]);const char*const*lines=help_pages[selected];for(int i=0;lines[i]&&8+i<rows-2;i++)clipped(8+i,2,cols-4,"%s",lines[i]);line_rule(rows-2,NULL);attron(COLOR_PAIR(COLOR_MUTED));clipped(rows-1,2,cols-4,"j/k・上下: 項目選択    Esc / ?: 閉じる");attroff(COLOR_PAIR(COLOR_MUTED));refresh();}
...
int main(int argc,char**argv){
    ...
    int open_selected=0,catch_selected=0,help_selected=0;char last_search[1024]="";bool search_active=false,search_whole_word=false,done=false;
    while(!done){
        ...
        if(view==VIEW_MAIN){draw_main(&g,&src,cursor,source_col,top,hscroll,mode,code_view,focus,var_rows,var_count,var_top,var_selected,reg_selected,reg_top,asm_selected,asm_top,stack_selected,last_search,search_whole_word,project_root,history.index,history.count);refresh();}else if(view==VIEW_HELP)draw_help(help_selected);else if(view==VIEW_OUTPUT)draw_output(&g);else if(view==VIEW_ADDRESS)draw_address(&address_panel);else if(view==VIEW_REGISTER)draw_register_panel(&register_panel);else if(view==VIEW_FUNCTIONS)draw_function_dialog(&g,&function_dialog);else if(view==VIEW_OPEN_MENU)draw_open_menu(open_selected);else if(view==VIEW_OPEN_FILES)draw_file_dialog(&g,&file_dialog);else if(view==VIEW_EDIT_VALUE)draw_edit_dialog(&edit_dialog);else if(view==VIEW_FORCE_RETURN)draw_force_return_dialog(&force_return_dialog);else if(view==VIEW_CATCH_MENU)draw_catch_menu(catch_selected);else if(view==VIEW_CATCH_SEARCH)draw_catch_search(&g,&catch_dialog);else draw_breaks(&g,bsel,project_root);
        int ch=getch();
        if(ch==ERR)continue;
        ...
        if(view==VIEW_HELP){if(ch==27||ch=='?'||ch=='h')view=VIEW_MAIN;else if((ch=='j'||ch==KEY_DOWN)&&help_selected+1<HELP_TOPIC_COUNT)help_selected++;else if((ch=='k'||ch==KEY_UP)&&help_selected>0)help_selected--;else if(ch==KEY_NPAGE){help_selected+=3;if(help_selected>=HELP_TOPIC_COUNT)help_selected=HELP_TOPIC_COUNT-1;}else if(ch==KEY_PPAGE){help_selected-=3;if(help_selected<0)help_selected=0;}continue;}
        ...
        if(ch=='?'&&mode==MODE_GDB){help_selected=0;view=VIEW_HELP;continue;}
