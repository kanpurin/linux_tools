...
# 表示言語を指定する場合
./gd/gd --lang ja ./program arg1 arg2
```
...
表示言語は`--lang auto|ja|en`で指定できます。既定値の`auto`は、UTF-8の日本語
ロケールでは日本語、それ以外では英語を使用します。起動後はGDB操作モードで`L`を押すと日本語／英語を
切り替えられます。日本語表示にはUTF-8ロケールが必要です。

日本語表示では、メイン画面、Help、停止条件一覧、各種ダイアログ、操作結果を日本語化します。
関数名、変数名、GDBが返す式や値、アセンブリ命令などの解析対象データは原文のまま表示します。
...
読み取り専用の**VIM操作モード**を切り替え、GDB操作モードでは`L`で表示言語を切り替えます。
