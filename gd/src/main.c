...
static void layout_heights(int rows,int*source_h,int*variables_h,int*stack_h){int available=rows-8;*source_h=available*55/100;if(*source_h<4)*source_h=4;int rest=available-*source_h;*variables_h=rest/2;if(*variables_h<3)*variables_h=3;*stack_h=rest-*variables_h;if(*stack_h<2)*stack_h=2;while(3+*source_h+1+*variables_h+1+*stack_h>rows-3){if(*source_h>4)(*source_h)--;else if(*variables_h>3)(*variables_h)--;else if(*stack_h>2)(*stack_h)--;else break;}}
...
static void draw_main(Gdb*g,Source*s,int cursor,int column,int top,int hscroll,InputMode mode,CodeView code_view,PaneFocus focus,VarRow*var_rows,int var_count,int var_top,int var_selected,int reg_selected,int reg_top,int asm_selected,int asm_top,int stack_selected,const char*search_text,bool search_whole_word,const char*project_root,int history_index,int history_count){int rows,cols,code_h,middle_h,stack_h;getmaxyx(stdscr,rows,cols);layout_heights(rows,&code_h,&middle_h,&stack_h);if(code_view==CODE_ASSEMBLY)while(middle_h<6&&stack_h>2){middle_h++;stack_h--;}erase();attron(A_BOLD|COLOR_PAIR(COLOR_INFO));clipped(0,1,cols-2,"gd [%s] [%s] [%s]",mode==MODE_VIM?"VIM":"GDB",code_view==CODE_ASSEMBLY?"ASM":"SRC",focus_name(focus));attroff(A_BOLD|COLOR_PAIR(COLOR_INFO));clipped(0,27,cols-28,"| %-11s | Exec frame #%d %s%s%s",state_name(g->state),g->selected_frame,g->function[0]?g->function:"-",g->source_available?":":" @ ",g->source_available?(char[32]){0}:g->pc);if(g->source_available){char where[64];snprintf(where,sizeof(where),"%d",g->line);clipped(0,cols>20?cols-20:0,19,"Exec line %s",where);}draw_stop_summary(g,cols);if(code_view==CODE_ASSEMBLY)draw_disassembly(g,asm_selected,asm_top,focus,code_h,cols);else draw_source_code(g,s,cursor,column,top,hscroll,mode,focus,search_text,search_whole_word,project_root,history_index,history_count,code_h,cols);int y=3+code_h+1;bool call_selected=asm_selected>=0&&asm_selected<g->instruction_count&&g->instructions[asm_selected].call;if(code_view==CODE_ASSEMBLY)draw_registers(g,reg_selected,reg_top,focus,y,middle_h,cols,call_selected);else draw_variables(g,var_rows,var_count,var_top,var_selected,focus,y,middle_h,cols);y+=middle_h+1;draw_stack(g,stack_selected,focus,y,stack_h,cols,project_root);line_rule(rows-3,NULL);if(focus==PANE_VARIABLES)clipped(rows-2,1,cols-2,"Variables  Enter Expand  p Value  a Address  E Edit  w Watch  B CondBreak  C Catch");else if(focus==PANE_REGISTERS)clipped(rows-2,1,cols-2,"Registers  j/k Select  p Details  E Edit  R ForceReturn  C Catch");else if(focus==PANE_STACK)clipped(rows-2,1,cols-2,"Stack  j/k Select  Enter Select frame  R ForceReturn  C Catch");else if(code_view==CODE_ASSEMBLY)clipped(rows-2,1,cols-2,"ASM  i/I Step  b AddrBreak  F FuncBreak  C Catch  o Open");else if(mode==MODE_GDB)clipped(rows-2,1,cols-2,"GDB  b/B Break  F Function  C Catch  L Stops  e Exec");int status_color=!strncmp(g->message,"ERROR",5)?COLOR_ERROR:strstr(g->message,"Watchpoint")||strstr(g->message,"watchpoint")?COLOR_WATCH:strstr(g->message,"Catch")||strstr(g->message,"Caught")?COLOR_COND:COLOR_INFO;attron(COLOR_PAIR(status_color));clipped(rows-1,1,cols-2,"%s",g->message);attroff(COLOR_PAIR(status_color));}
static void draw_help(void){erase();int y=0;attron(A_BOLD);mvaddstr(y++,2,"gd modes and commands");attroff(A_BOLD);const char*lines[]={"","Tab / Shift-Tab  表示中モードのペインを前後移動","F2  VIMモード / GDBモードを切り替える","o   Function / Fileからソースを開く（実行位置は変更しない）","e   実行位置へ戻る   d  Source / Disassembly切り替え","Ctrl-o / ]  閲覧履歴を戻る / 進む（Ctrl-iは端末上でTabと同一）","O   Program output","","Modify（デバッグ中プロセスのみ。ソースは変更しない）","  Variables: E  選択変数・メンバの値を変更","  Registers: E  選択レジスタの値を変更","  Execution: R  現在の関数を強制return","","Stop conditions","  b   現在のソース行 / 命令アドレスにBreakpoint","  B   条件付きBreakpoint","  F   関数名を検索してFunction Breakpoint","  C   Catchpoint（syscall/signal/fork/vfork/exec/load）","  w   Watchpoint    L   Stop conditions一覧","  REFERENCE ONLYソースでは行Breakpointを設定しない","","デバッグ情報なし / Disassembly","  i   1命令進める（stepi）   I   callを飛び越す（nexti）","  j/k 命令選択  b 選択アドレスにBreakpoint","","Stackペイン","  j/k  フレーム選択   Enter  フレームを選択","  [P] project内   [X] 外部   [A] address only","","Variablesペイン","  Enter 展開/折りたたみ  p 詳細  a Address  E Edit  w Watch","","VIMモード（Sourceのみ・読み取り専用）","  h/j/k/l, w/b/e, 0/^/$, gg/G, gd/gD, /?, n/N, *#, %","","GDBモード","  r 実行  n next  s step  c continue  f finish","","q 終了   Esc / h ヘルプを閉じる",NULL};for(int i=0;lines[i]&&y<getmaxy(stdscr);i++)mvaddstr(y++,2,lines[i]);refresh();}
...
int main(int argc,char**argv){
    ...
    while(!done){
        ...
        if(view==VIEW_MAIN){draw_main(&g,&src,cursor,source_col,top,hscroll,mode,code_view,focus,var_rows,var_count,var_top,var_selected,reg_selected,reg_top,asm_selected,asm_top,stack_selected,last_search,search_whole_word,project_root,history.index,history.count);refresh();}else if(view==VIEW_HELP)draw_help();else if(view==VIEW_OUTPUT)draw_output(&g);else if(view==VIEW_ADDRESS)draw_address(&address_panel);else if(view==VIEW_REGISTER)draw_register_panel(&register_panel);else if(view==VIEW_FUNCTIONS)draw_function_dialog(&g,&function_dialog);else if(view==VIEW_OPEN_MENU)draw_open_menu(open_selected);else if(view==VIEW_OPEN_FILES)draw_file_dialog(&g,&file_dialog);else if(view==VIEW_EDIT_VALUE)draw_edit_dialog(&edit_dialog);else if(view==VIEW_FORCE_RETURN)draw_force_return_dialog(&force_return_dialog);else if(view==VIEW_CATCH_MENU)draw_catch_menu(catch_selected);else if(view==VIEW_CATCH_SEARCH)draw_catch_search(&g,&catch_dialog);else draw_breaks(&g,bsel,project_root);
        int ch=getch();
        if(ch==ERR)continue;
