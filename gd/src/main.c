...
static void draw_help_popup(void){static const char*english[]={"Execution","  r / c       Run / Continue","  n / s       Next / Step into","  f           Finish current function and return to caller","              caller has source -> Source, otherwise Assembly","  i / I       Step instruction / Next instruction","  R           Force return (confirmation required)","","View / History","  Tab         Switch pane    Shift-Tab: reverse","  d / e       Source/Assembly / Return to execution position","  o           Open Function / File","  [ / Ctrl-o  View history back    ]: forward","              history only changes the view; f executes","","Stop conditions","  b / B / F   Breakpoint / Conditional / Function","  C / w / L   Catchpoint / Watchpoint / Stop list","","Panes","  Variables   Enter expand, p value, a address, E edit","  Registers   p details, E edit","  Stack       Enter selects frame and opens its location","","Modes / Other","  F2          GDB / read-only VIM mode","  F3          Switch UI language","  VIM         h/j/k/l, /?, n/N, gd/gD, *#, %","  O           Program output","  ?           Help    q: Quit",NULL};static const char*japanese[]={"実行","  r / c       実行 / 継続","  n / s       次の行 / 関数内へステップ","  f           現在の関数から戻るまで実行","              呼出し元にソースあり -> Source、なし -> Assembly","  i / I       1命令実行 / 次の命令へ","  R           強制return（確認あり）","","表示 / 履歴","  Tab         ペイン切替    Shift-Tab: 逆方向","  d / e       Source/Assembly切替 / 実行位置へ戻る","  o           関数 / ファイルを開く","  [ / Ctrl-o  閲覧履歴を戻る    ]: 進む","              履歴は表示のみ変更。fはプログラムを実行","","停止条件","  b / B / F   Breakpoint / 条件付き / 関数","  C / w / L   Catchpoint / Watchpoint / 停止条件一覧","","ペイン","  変数         Enter 展開、p 値、a アドレス、E 編集","  レジスタ     p 詳細、E 編集","  スタック     Enterでフレーム選択と位置移動","","モード / その他","  F2          GDB / 読み取り専用VIMモード","  F3          表示言語を切り替え","  VIM         h/j/k/l、/?、n/N、gd/gD、*#、%","  O           プログラム出力","  ?           ヘルプ    q: 終了",NULL};const char**lines=ui_japanese?japanese:english;int count=0;while(lines[count])count++;WINDOW*window=create_popup(tui_text("Help","ヘルプ"),count+2,76);if(!window)return;int height,width;getmaxyx(window,height,width);for(int i=0;i<count&&i+1<height-1;i++){if(lines[i][0]&&!isspace((unsigned char)lines[i][0]))wattron(window,A_BOLD);add_clipped(window,i+1,2,lines[i],width-4);wattroff(window,A_BOLD);}wnoutrefresh(window);delwin(window);}
...
static void draw_breaks(Gdb *g, int selected, const char *project_root) {
    ...
    clipped(rows - 1, 1, cols - 2,
            "%s",tui_text("j/k Select  d Delete  e Toggle  Enter Jump  Esc/L Back","j/k 選択  d 削除  e 有効/無効  Enter 移動  Esc/L 戻る"));
    attroff(COLOR_PAIR(COLOR_MUTED));
    refresh();
}
...
int main(int argc,char**argv){
    ...
    while(!done){
        ...
        if(view!=VIEW_MAIN){
            if(ch==27||(view==VIEW_OUTPUT&&ch=='O')||(view==VIEW_BREAKS&&ch=='L')||(view==VIEW_ADDRESS&&(ch=='a'||ch=='\n'||ch==KEY_ENTER))||(view==VIEW_REGISTER&&(ch=='p'||ch=='\n'||ch==KEY_ENTER))){view=VIEW_MAIN;continue;}
        ...
        if(ch==KEY_F(3)){if(!ui_japanese&&!locale_is_utf8())snprintf(g.message,sizeof(g.message),"Japanese UI requires a UTF-8 locale");else{ui_japanese=!ui_japanese;g.japanese=ui_japanese;snprintf(g.message,sizeof(g.message),"%s",tui_text("Display language changed to English","表示言語を日本語に変更しました"));}continue;}
